...
アプリケーションの起動方法
----------------------
1. 開発モード（Pythonから直接実行）:
   python main.py

2. 実行ファイル（.exe）を使用:
   - dist/todo_app.exeをダブルクリックで実行
   - 他のコンピュータでも、todo_app.exeをコピーするだけで実行可能
   - Python環境は不要

注意事項
-------
- 実行ファイル版（.exe）でも、タスクデータは同じようにtasks.jsonに保存されます
- 実行ファイルを別のフォルダに移動する場合は、既存のtasks.jsonも一緒に移動することで、タスクデータを引き継ぐことができます
